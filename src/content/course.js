...
const unit2 = {
  ...
  learn: {
    chunks: [
      {
        id: 'l1', type: 'hook',
        text: 'יונתן הקטן, רץ בבוקר... — מה המילה הבאה?',
        subtext: 'עכשיו נסה/י: "יונתן הקטן, רץ בערב..." — אותן מילות פתיחה, תוצאה שונה. למה?',
        buttonLabel: 'כי ה-AI לא "מבין" — הוא מנבא ←',
      },
      {
        id: 'l2', type: 'concept',
  ...
  see: {
    ...
    observations: [
      {
        id: 's1', title: '🎬 איך מסבירים LLM לילדים?',
        visual: { type: 'youtube', videoId: 'K3TiN6wAKO0', alt: 'הסבר LLM לילדים — ניבוי מילה הבאה' },
        question: 'הסרטון מסביר שה-AI מנחש מילה אחת בכל פעם. מה יקרה אם יטעה במילה אחת?',
        reveal: '"יונתן הקטן, רץ בבוקר..." ו-"יונתן הקטן, רץ בערב..." — ההמשך שונה לגמרי! המודל מחשב הסתברויות על בסיס כל ההקשר. טעות אחת → ניבויים שונים → טקסט שונה לחלוטין.',
      },
      {
        id: 's1b', title: '🔬 LLM — סרטון למתקדמים',
        visual: { type: 'youtube', videoId: 'kopoLzvh5jY', alt: 'הסבר מעמיק על Large Language Models' },
        question: 'מה ההבדל בין "הבנה" לבין "ניבוי" — האם ה-AI "מבין" את הטקסט שהוא מייצר?',
        reveal: 'ה-AI לא "מבין" — הוא מנבא. הוא ראה מיליארדי משפטים ולמד: אחרי "יונתן הקטן, רץ בבוקר" — "לבית הספר" הופיעה הכי הרבה. זה לא ידע — זו סטטיסטיקה ענקית.',
      },
      {
...
// ─────────────────────────────────────────────────────────────────────────────
// UNIT DEMO — AI בעולם האמיתי: הצלחות, כישלונות, ומה שביניהם
// Standalone showcase unit — real-world examples + full Learn→See→Do cycle
// ─────────────────────────────────────────────────────────────────────────────
const unitDemo = {
  id: 'unit-demo',
  ...
  isDemo: true,
  competencies: ['researcher'],
  summary: 'סופרמרקט שנפתח רק לחייכנים. מצלמה שלומדת שפת סימנים. 50 מיליון ציורי חתול. — נבין מתי AI מצליח, מתי נכשל, ולמה.',
  duration: '40 דקות',
    ...
    chunks: [
      {
        id: 'l1', type: 'video',
        videoId: 'Bs5nyvkAix8',
        alt: 'סופרמרקט דנמרק — דלת נפתחת רק לחייכנים',
        preText: '👀 צפה/י בסרטון הזה. מה ה-AI הזה לדעתך "רואה"?',
        subtext: '💡 לא "אושר" ולא "שמחה" — הוא מזהה דפוסים של פיקסלים שמתאימים לחיוך.',
        buttonLabel: 'איך הוא למד? ←',
      },
      {
        id: 'l2', type: 'concept',
        heading: 'AI לא "מבין" — הוא מזהה דפוסים',
        bullets: [
          '🧁 מאפה: חום + עגול + כתמים כהות = קטגוריה A',
          '🐶 כלבלב: חום + עגול + כתמים כהות = קטגוריה A?!',
          '📸 AI שאומן רק על תמונות דומות לא יכול להפריד ביניהן',
        ],
        subtext: 'הנתונים קובעים מה ה-AI לומד. אם הנתונים מוטים — גם המודל מוטה.',
        buttonLabel: 'מתי זה מצליח ומתי נכשל? ←',
      },
      {
        id: 'l3', type: 'scenario-quiz',
        heading: 'מתי AI מצליח? מתי נכשל? — נחשו!',
        buttonLabel: 'מושג המפתח ←',
        scenarios: [
          { text: '📸 נתוני אימון: גילאים שונים, עורות שונים, תאורות שונות, זוויות שונות', result: 'success', explain: 'מגוון = חוסן. המודל מוכן לעולם האמיתי.' },
          { text: '📸 נתוני אימון: רק פנים צעירות אירופאיות באור סטודיו', result: 'fail', explain: 'הטיה בנתונים → כישלון עם פנים שלא ראה. Garbage in.' },
          { text: '✅ המשימה: "חיוך / לא חיוך" — כן או לא, ברור', result: 'success', explain: 'משימה בינארית וברורה = AI יכול לפתור.' },
          { text: '❓ המשימה: "כמה שמח/ה מ-1 עד 10?"', result: 'fail', explain: 'עמומה מדי — כל אחד מגדיר "7" אחרת. הנתונים יסתרו.' },
          { text: '🧪 בדיקה על נתונים חדשים שהמודל מעולם לא ראה', result: 'success', explain: 'כך יודעים אם המודל הכליל — לא שינן.' },
          { text: '🧪 בדיקה רק על הנתונים שאומנו עליהם', result: 'fail', explain: 'כמו לשאול סטודנט על שאלות שכבר ראה. לא מודד הבנה אמיתית.' },
        ],
      },
      {
        id: 'l4', type: 'keyterm',
        term: 'Training Data',
        termHe: 'נתוני אימון',
        definition: 'הדוגמאות שמהן AI לומד. כמות + מגוון + דיוק התוויות = איכות המודל. זבל פנימה — זבל החוצה.',
        buttonLabel: 'נראה דוגמאות אמיתיות ←',
      },
    ],
  },
  ...
  see: {
    observations: [
      {
        id: 's1',
        title: '🧁 מאפה או כלבלב? — תהיה/י ה-AI',
        visual: { type: 'mini-game', gameComponent: 'MuffinOrDog' },
        question: 'באילו מקרים התקשית להחליט? מה גרם לבלבול?',
        reveal: 'AI שאומן לזהות כלבלבים על תמונות מלמעלה יתבלבל בדיוק כמוך. הוא לא "מבין" כלבלב — הוא מזהה: חום + עגול + כתמים כהות = קטגוריה. כשמאפה חולק את אותם פיקסלים, אין לו דרך להפריד. זה לא באג — זה מה שנתוני האימון לימדו אותו.',
      },
      {
        id: 's2',
        title: '🐱 נתוני אימון מוטים — כישלון בלתי נמנע',
        visual: { type: 'mini-game', gameComponent: 'BiasDemo' },
        question: 'מה היית משנה בנתוני האימון כדי שהמודל יצליח?',
        reveal: 'המודל לא למד "חתול" — הוא למד "בהיר + פרוותי". חתול שחור? "לא חתול". כלב לבן? "אולי חתול?". הפתרון: נתוני אימון מגוונים — חתולים מכל הצבעים, גזעים, זוויות ותאורות. Garbage in → Garbage out.',
      },
      {
        id: 's3',
        title: '🚪 הסופרמרקט המחייך — דנמרק',
        visual: { type: 'youtube', videoId: 'Bs5nyvkAix8', alt: 'סופרמרקט דנמרק פותח דלתות רק לאנשים מחייכים' },
        question: 'איזה נתוני אימון צריך המודל הזה? מה יקרה אם מישהו שמח אבל לא מחייך?',
        reveal: 'המודל אומן על תמונות של פנים מחייכות ולא מחייכות מאלפי אנשים. הצלחה: חיוך ברור בתאורה טובה. כישלון: מסכות, חיוך מוסתר, תאורה חלשה, פנים שלא מופיעות בנתוני האימון. המשימה הצרה "חיוך/לא חיוך" — בדיוק מה שאפשר ל-AI לעשות.',
      },
      {
        id: 's4',
        title: '🤟 שפת סימנים — כשנתוני האימון הם אתה/ן',
        visual: { type: 'youtube', videoId: 'E7X9CWn_ElQ', alt: 'Teachable Machine לוקחת שפת סימנים — אדם מאמן AI בעצמו' },
        question: 'מי סיפק את נתוני האימון כאן? מה ההגבלה של מודל שאמנת רק על עצמך?',
        reveal: 'היוצר/ת "צילמ/ה" עצמ/ם עושה כל סימן עשרות פעמים. המודל למד את הסימנים שלהם — לא של כולם. אם מישהו אחר ינסה? כנראה שהמודל יתבלבל. זו הסיבה שמודלים מסחריים אומנים על מיליוני אנשים שונים.',
      },
      {
        id: 's5',
        title: '🎨 50 מיליון ציורי "חתול" — מה AI לומד מהם?',
        visual: { type: 'mini-game', gameComponent: 'QuickDrawGallery' },
        question: 'כמה הציורים שונים אחד מהשני? מה הדבר המשותף שה-AI צריך ללמוד לזהות?',
        reveal: 'AI לא שינן ציור מושלם. הוא מצא: אוזניים משולשות, ראש עגול, שפם — גם בציורים גרועים. ככל שיש יותר ציורים מגוונים, ה-AI מפתח "הבנה" שיכולה לזהות גם ציורים שמעולם לא ראה. זה כוח הנתונים הגדולים.',
      },
      {
        id: 's6',
        title: '🛠 אמן/י מודל — כמו הצוות שבנה את דלת הסופרמרקט',
        visual: { type: 'mini-game', gameComponent: 'TeachableSim', gameProps: { preset: 'gesture' } },
        question: 'מה היה שונה אם היית מצלמ/ת רק יד אחת, בזווית אחת? מה חסר בנתונים?',
        reveal: 'מודל שאומן רק על יד אחת מזווית אחת → יתבלבל עם יד שמאל, כפפה, תאורה שונה. הסופרמרקט הדני אמן על אלפי אנשים שונים. מגוון בנתוני האימון = חוסן של המודל בעולם האמיתי.',
      },
    ],
  },

  practice: {
    intro: 'עצב/י מערכת AI משלך',
    steps: [
      {
        id: 'p0',
        instruction: '🛠 אמן/י מודל AI אמיתי! פתח/י את Teachable Machine — צלמ/י דוגמאות של שתי קטגוריות שתבחר/י (למשל: יד פתוחה / אגרוף), ואמן/י. ראה/י כמה דוגמאות צריך עד שהמודל מדויק.',
        action: 'launch-task',
        url: 'https://teachablemachine.withgoogle.com/train',
        launchLabel: '🚀 פתח/י Teachable Machine ↗',
        doneLabel: '✅ אימנתי מודל — סיימתי',
        successText: 'כל הכבוד! חזרת עם ניסיון ישיר.',
        instructions: [
          'לחץ/י על "Image Project" ← "Standard image model"',
          'שנה/י את שם Class 1 ו-Class 2 לקטגוריות שלך (למשל: "יד פתוחה" / "אגרוף")',
          'לחץ/י "Webcam" וצלמ/י לפחות 30 תמונות לכל קטגוריה',
          'לחץ/י "Train Model" — המתן/י עד שה-Training גמור',
          'בדוק/י: הגדל/י את הקטגוריות, שנה/י זווית — מה משפיע על הדיוק?',
        ],
      },
      {
        id: 'p1',
        instruction: 'בחר/י בעיה מהחיים שלך שAI יכול לפתור — כמו הסופרמרקט הדני. תאר/י: מה יהיו נתוני האימון? מאיפה תאסוף/י אותם?',
        action: 'text',
        placeholder: 'הבעיה שלי: ___. ה-AI יצטרך לזהות: ___. אאסוף נתוני אימון מ: ___ — בערך ___ דוגמאות.',
      },
      {
        id: 'p2',
        instruction: 'תחשוב/י על שני מקרים שבהם המודל שלך ייכשל. מה הגורם לכישלון — הנתונים? המשימה? משהו אחר?',
        action: 'text',
        placeholder: 'כישלון 1: המודל יתבלבל כאשר ___ כי ___. כישלון 2: ___',
      },
      {
        id: 'p3',
        instruction: 'ה-AI שבנה הצוות הדני פעל רק 3 שעות לפני שהפסיקו את הניסוי — למה? מה היית עושה אחרת?',
        action: 'text',
        placeholder: 'אני חושב/ת שהפסיקו כי ___ . הייתי משנה ___',
      },
    ],
  },

  document: {
    question: 'בחר/י דוגמה אחת של AI מהעולם האמיתי (לא מהשיעור). תאר/י: מה נתוני האימון שלו? מתי הוא מצליח? מתי נכשל? ומה היה אפשר לשפר?',
    hint: 'השתמש/י במילים: נתוני אימון, מגוון, כישלון',
    artifactType: 'reflection',
    requiredKeywords: ['נתוני אימון', 'מגוון', 'כישלון'],
    placeholder: 'הדוגמה שלי: ___ . נתוני האימון שצריכים: ___ . מצליח כאשר: ___ . נכשל כאשר: ___ . שיפור אפשרי: ___',
  },
...
export const COURSE = {
  ...
  units: [unitDemo, unit1, unit2, unit3, unit4, unit5, unit6, unit7, unit8, unit9, unit10, unit11, unit12],
}
